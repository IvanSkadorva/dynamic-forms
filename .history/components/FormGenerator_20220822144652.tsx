...
export const FormGenerator: React.FC<FormGeneratorProps> = ({ fields }) => {
  ...
  const getDisablingRules = () => {
    const disablingFields = new Map()
    fields.forEach(({ id, validation }, i) => {
      if (validation?.disablesIfTrue) {
        disablingFields.set(disablingFields[id], validation?.disablesIfTrue)
      }
    })
  }
  ...
  const handleChange = ({ currentTarget }) => {
    affectingField
      ? affectedField.setAttribute('disabled', true)
      : affectedField.setAttribute('disabled', false)
  }
