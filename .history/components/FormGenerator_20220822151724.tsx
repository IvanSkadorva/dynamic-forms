...
export const FormGenerator: React.FC<FormGeneratorProps> = ({ fields }) => {
  ...
  const handleChange = ({ currentTarget }) => {
    console.log('hhiiii', disablingFields)

    const form = currentTarget
    for (const [key, value] of disablingFields) {
      console.log(key, value, form[key], form['endDate'])
      form[key].value
        ? form[value].setAttributes({ disabled: true })
        : form[value].setAttributes({ disabled: false })
    }
  }
